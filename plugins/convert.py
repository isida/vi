...
currency_conv_list = ['ATS', 'AUD', 'BEF', 'BYR', 'CAD', 'CHF', 'CNY', 'DEM',
		'DKK', 'EEK', 'EGP', 'ESP', 'EUR', 'FIM', 'FRF', 'GBP', 'GRD', 'IEP',
		'ISK', 'ITL', 'JPY', 'KGS', 'KWD', 'KZT', 'LTL', 'NLG', 'NOK', 'PTE',
		'RUB', 'SDR', 'SEK', 'SGD', 'TRL', 'TRY', 'UAH', 'USD', 'XDR', 'YUN']

def cmd_currency_converter(raw_in, text):
	msg = 'Error in parameters. Read the help about command.'
	text = text.strip().upper()
	if text == 'LIST':
		msg = 'Available values:\n%s' % ', '.join(currency_conv_list).replace('RUB', 'RUR')
	else:
		repl_curr = ((u'RUR', 'RUB'), (u'€', 'EUR'), ('$', 'USD'), (u'¥', 'JPY'), (u'£', 'GBP'), (',', '.'))
		for tmp in repl_curr:
			text = text.replace(tmp[0], ' %s ' % tmp[1])
		c_from = ''
		c_to = ''
		c_summ = ''
		...
		if c_from and c_to and c_summ:
			...
			url = 'http://quote.rbc.ru/cgi-bin/conv/external/converter.shtml?source=cb.0&tid_from=%s&commission=1.0&tid_to=%s&summa=%s&day=%s&mon=%s&year=%s'
			url = url % (c_from, c_to, c_summ, date[2], mname[date[1]-1], date[0])
			body = load_page(url)
			try:
				...
				curr_list = {}
			try:
				body = body.split('<table id="rTable" class="table">', 1)[1]
			except:
				body = ''
			...
			if mt != []:
				...
				if curr_list.has_key(mt[0]):
					frv = curr_list[mt[0]]
				else:
					frv = mt[0]
				if curr_list.has_key(mt[3]):
					tov = curr_list[mt[3]]
				else:
					tov = mt[3]
				msg = '%s %s (%s) = %s %s (%s)\n1 %s = %s %s' % (c_summ, c_from, frv, mt[4], c_to, tov, c_from, mt[2], c_to)
				msg = msg.replace('RUB', 'RUR')
	send_msg(raw_in, msg)

commands = [['convert', cmd_currency_converter, False, 'less', 'Currency converter. From to count. List']]
